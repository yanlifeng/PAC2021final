...
void Result::print() {
    ...
    cout << "mapped_reads:\t" << mBarcodeProcessor->mMapToSlideRead << "\t"
         << (double) mBarcodeProcessor->mMapToSlideRead / (double) (mTotalRead - mFxiedFilterRead) * 100 << "%" << endl;
    double overlapReadsWithMisRate =
            (double) mBarcodeProcessor->overlapReadsWithMis / (double) mBarcodeProcessor->totalReads * 100;
    double overlapReadsWithNRate =
            (double) mBarcodeProcessor->overlapReadsWithN / (double) mBarcodeProcessor->totalReads * 100;
    cout << "barcode_exactlyOverlap_reads:\t" << mBarcodeProcessor->overlapReads << "\t"
         << (double) mBarcodeProcessor->overlapReads / (double) mBarcodeProcessor->totalReads * 100 << "%" << endl;
    cout << "barcode_misOverlap_reads:\t" << mBarcodeProcessor->overlapReadsWithMis << "\t" << overlapReadsWithMisRate
         << "%" << endl;
    cout << "barcode_withN_reads:\t" << mBarcodeProcessor->overlapReadsWithN << "\t" << overlapReadsWithNRate << "%"
         << endl;
    double barcodeQ10 =
            (double) mBarcodeProcessor->barcodeQ10 / (double) (mBarcodeProcessor->totalReads * mOptions->barcodeLen) *
            100;
    double barcodeQ20 =
            (double) mBarcodeProcessor->barcodeQ20 / (double) (mBarcodeProcessor->totalReads * mOptions->barcodeLen) *
            100;
    double barcodeQ30 =
            (double) mBarcodeProcessor->barcodeQ30 / (double) (mBarcodeProcessor->totalReads * mOptions->barcodeLen) *
            100;
    double umiQ10 = 0;
    double umiQ20 = 0;
    double umiQ30 = 0;
    if (mOptions->transBarcodeToPos.umiLen > 0) {
        umiQ10 = (double) mBarcodeProcessor->umiQ10 /
                 (double) (mBarcodeProcessor->mMapToSlideRead * mOptions->transBarcodeToPos.umiLen) * 100;
        umiQ20 = (double) mBarcodeProcessor->umiQ20 /
                 (double) (mBarcodeProcessor->mMapToSlideRead * mOptions->transBarcodeToPos.umiLen) * 100;
        umiQ30 = (double) mBarcodeProcessor->umiQ30 /
                 (double) (mBarcodeProcessor->mMapToSlideRead * mOptions->transBarcodeToPos.umiLen) * 100;
    }
    ...
    long umiTotalFilterReads = mBarcodeProcessor->umiNFilterReads + mBarcodeProcessor->umiPloyAFilterReads +
                               mBarcodeProcessor->umiQ10FilterReads;
    ...
    double umiPolyAFilterRate =
            (double) mBarcodeProcessor->umiPloyAFilterReads / (double) mBarcodeProcessor->totalReads * 100;
    double umiQ10FilterRate =
            (double) mBarcodeProcessor->umiQ10FilterReads / (double) mBarcodeProcessor->totalReads * 100;
    ...
    cout << "umi_with_polyA_reads:\t" << mBarcodeProcessor->umiPloyAFilterReads << "\t" << umiPolyAFilterRate << "%"
         << endl;
    cout << "umi_with_low_quality_base_reads:\t" << mBarcodeProcessor->umiQ10FilterReads << "\t" << umiQ10FilterRate
         << "%" << endl;
}
...
void Result::setBarcodeProcessorHash(robin_hood::unordered_map<uint64, Position1> **bpmap_hash) {
    mBarcodeProcessor = new BarcodeProcessor(mOptions, bpmap_hash);
}

void Result::setBarcodeProcessorHashIndex(robin_hood::unordered_map<uint32, uint32> **bpmap_hash_index,
                                          Position1 *position1_index){
    mBarcodeProcessor = new BarcodeProcessor(mOptions, bpmap_hash_index,position1_index);
}

void Result::setBarcodeProcessorHashTable(int* bpmap_head, int* bpmap_nxt,uint64* bpmap_key,int* bpmap_value,Position1* position_index){
